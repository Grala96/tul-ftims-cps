...
public class UnitJump extends Signal {
    ...
    public UnitJump(Double amplitude, Double startTime, Double duration, Double samplingFrequency, Double ts) {
        super(amplitude, startTime, duration, samplingFrequency, 0.0, signalType.CONTINIOUS);
        this.ts = ts;
    ...
    public UnitJump(double amplitude, double startTime, double duration, double samplingFrequency, double ts) {
        super(Double.valueOf(amplitude), Double.valueOf(startTime), Double.valueOf(duration), Double.valueOf(samplingFrequency), Double.valueOf(0.0), signalType.CONTINIOUS);
        this.ts = ts;
        this.generate(getSamples());
        this.calculateValues();
    }
